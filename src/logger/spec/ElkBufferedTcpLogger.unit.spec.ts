...
describe("ElkBufferedTcpLogger unittest", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(elkRestApi, "postManyDocuments").mockResolvedValue(undefined)
  ...
  test("should process logs and send them to ElkRestApi", async () => {
    ...
    logs.forEach((log) => elkBufferedApi.log(log))

    // Wait for the buffer interval to trigger the processing
    await new Promise((resolve) => setTimeout(resolve, 1500))

    expect(elkRestApi.postManyDocuments).toHaveBeenCalledTimes(1)
    expect(elkRestApi.postManyDocuments).toHaveBeenCalledWith(logs)
  })
  test("should process logs and send them to ElkRestApi variation", async () => {
    ...
    expect(elkRestApi.postManyDocuments).toHaveBeenCalledTimes(0)
    // Wait for the buffer interval to trigger the processing
