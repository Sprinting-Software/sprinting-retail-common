...
import { ElkV7Config, LibConfig } from "../config/interface/LibConfig"
...
export class LegacyLoggerService extends LoggerService implements OnApplicationShutdown {
  ...
  constructor(
    private readonly config: LibConfig & ElkV7Config,
    transports: any[] = [],
  ...
  private initTcpLogger(config: LibConfig & ElkV7Config, logType: "event" | "error") {
  ...
  private initTcpSender(config: LibConfig & ElkV7Config) {
    if (!this.config.elkRestApi) return
    const yyyyww = getYearAndWeek()
  ...
  info(fileName: string, message: string, messageData?: Record<string, any>) {
    ...
    LegacyLoggerService.logger.info(doc)
  }

  log(message: any, context?: string): void {
    this.info(context || "Application", String(message))
  }
  ...
  verbose(message: any, context?: string): void {
    this.debug(context || "Application", message)
  }

  debug(fileName: string, message: any, messageData?: Record<string, any>) {
    LegacyLoggerService.logger.debug(
  ...
  warn(fileName: string, message: string, messageData?: Record<string, any>) {
    LegacyLoggerService.logger.warn(
      this.formatMessage(fileName, LogLevel.warn, message, messageData, this.getAsyncContext())
    )
  }
  ...
  event(
    ...
    if (this.config?.elkRestApi?.useForEvents && this.tcpLoggerEvents) {
      ...
      LegacyLoggerService.loggerConsoleOnly.info(logMessage)
    } else {
      LegacyLoggerService.logger.info(logMessage)
    }
  }
  ...
  logError(error: Exception | Error | unknown, contextData?: Record<string, any>) {
    ...
    const fileName = LegacyLoggerService._getCallerFile()
    ...
    if (this.config.elkRestApi?.useForErrors && this.tcpLoggerErrors) {
      ...
      LegacyLoggerService.loggerConsoleOnly.error(formatedMessage)
    } else {
      LegacyLoggerService.logger.error(formatedMessage)
    }
  }
